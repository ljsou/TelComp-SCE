...
WireIt.Layer = function(options) {
    ...
    this.targetDataElement = [];
    this.sourceDataElement = [];
...
WireIt.Layer.prototype = {
    ...
    setOptions: function(options) {
        ...
        this.options.layerMap = YAHOO.lang.isUndefined(options.layerMap) ? true : options.layerMap;
...
//                    tgt: {moduleId: WireIt.indexOf(wire.terminal2.container, this.containers), terminalId: wire.terminal2.options.name}
        ...
        for (var i = 0; i < n; i++) {
            ...
            if (this.wires[i].wireID === '' + wid) {
                ...
                console.log("Data Mapping between " + this.wires[i].terminal1.container.options.title
                        ...
                        + "(" + this.wires[i].terminal2.container.options.idcomp + ") - dataFlow: " + this.wires[i].dataFlow + "done");
                this.eraseDataElements();
            }
        }
    },
    userDataMapping: function(wid) {
        ...
        for (var i = 0; i < n; i++) {
            ...
            //console.log(this.wires[i].wireID + "==" + wid);
            if (this.wires[i].wireID === '' + wid) {
                this.wires[i].options.color = 'rgb(244, 250, 88)';
                this.wires[i].options.bordercolor = 'rgb(254, 154, 46)';
                this.wires[i].redraw();
                this.wires[i].dataFlow = true;
                this.wires[i].sourceData = this.sourceDataElement;
                this.wires[i].targetData = this.targetDataElement;
